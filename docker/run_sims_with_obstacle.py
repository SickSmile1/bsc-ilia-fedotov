...
def init_mesh(mesh_file,canal_length=5.,canal_height=2.,obstacle_center_x=2.,obstacle_center_y=2.,obstacle_radius=2.5,
    ...
    ----------
    mesh_file : string
        String to save/load the mesh file
    canal_length : float
    ...
    d_in : float
        Mesh value near obstacle.
    d_out : float
        Mesh value far from obstacle.
    b_in : float
        Mesh value vertical center of the obstacle.
    b_out : float
        Mesh value far from vertical center of the obstacle.
    Returns: None
    --------
    """
    ...
    for boundary in boundaries:
        ...
        elif np.isclose(node_coords[0], canal_length, atol=tol):https://www.videoportal.uni-freiburg.de/
...
def print_msh(file, name, canal_height=2, canal_length=5):
    gmsh.open(file)
    gmsh.model.occ.synchronize()
...
def walls(x, canal_height=2.):
    return np.logical_or(np.isclose(x[1], 0), np.isclose(x[1], canal_height))

def inflow(x):
    return np.isclose(x[0], 0)

def outflow(x, canal_length=5.):
    return np.isclose(x[0], canal_length)

def u_exact(x, canal_height=2., canal_length=5., obstacle_radius=.5, obstacle_center_x=2.,
           obstacle_center_y=2.):
...
def calc_with_dolfin(filename,comm, canal_height, canal_length, obstacle_radius,
                     obstacle_center_x, obstacle_center_y, pressure=8.,num_steps=500, T=10):
    """
    Run FEM Calculation using dolfinx
    ...
    obstacle_center_y : float
        The y-coordinate position of the obstacle's center.
        For a wall-attached obstacle, this would be `H`. Default is 2.5.
    num_steps : int
    ...
    F1 -= dot(f, v) * dx
    print("defined u and p functions")
...
if __name__ == "__main__":
    ...
    """
    # run sims
    # filename,comm, canal_height, canal_length, obstacle_radius,
    #    obstacle_center_x, obstacle_center_y, pressure=8, num_steps=500, T=10
    u, p, V, mesh = calc_with_dolfin('r.5.msh',comm, 2.,5.,.5,2.,2.)
    flow, pressure = calculate_flow_and_pressure(u,p,V,mesh)
...
def calculate_flow_and_pressure(u, p, V, mesh):
    ...
    for x_pos in np.arange(x_min, x_max + interval, interval):
        ...
        slice_condition = ufl.conditional(ufl.ge(x[0], x_pos - 1e-6), 1.0, 0.0) * ufl.conditional(ufl.le(x[0], x_pos + 1e-6), 1.0, 0.0)
        
        # Calculate mass flow rate at the current slice
        mass_flow = dolfinx.fem.assemble_scalar(dolfinx.fem.form(u_sub * slice_condition * dx))
        
        # Calculate average pressure at the current slice
        pressure_avg = dolfinx.fem.assemble_scalar(dolfinx.fem.form(p * slice_condition * dx)) / dolfinx.fem.assemble_scalar(dolfinx.fem.form(slice_condition * dx))
...
def save_mfr(u_n, V, canal_height=2., canal_length=5., obstacle_center_x=2. ):
    ...
    # beginning_slice = conditional(le(x[0], 0.0 + .5), canal_height, 0.0)  # Near x = 0
    # middle_slice = conditional(ge(x[0], obstacle_center_x - 1e-6), canal_height, 0.0) * conditional(le(x[0], obstacle_center_x + 1e-6), 1.0, 0.0)  # Near x = L/2
    # end_slice = conditional(ge(x[0], canal_length - 1 - 1e-6), canal_height, 0.0)  
