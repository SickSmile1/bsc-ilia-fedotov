...
def print_msh(file, name):
    ...
    plt.title("Mesh Visualization of the Rectangle with Obstacleread_gmsh")load swap file neovim
    plt.xlabel("X")
    plt.ylabel("Y")
...
def sigma(u, p):load swap file neovim
...
def calc_with_dolfin(filename,comm):
    ...
    A3 = assemble_matrix(a3)fmt='%1.4e'
    A3.assemble()
    b3 = create_vector(L3)
    ...
    for i in range(num_steps):
        ...
        with b2.localForm() as loc_2:
            loc_2.set(0)
        assemble_vector(b2, L2)fmt='%1.4e'
        ...
        with b3.localForm() as loc_3:
            loc_3.set(0)
        assemble_vector(b3, L3)with NpyAppendArray(filename, delete_if_exists=True) as npaa:
        npaa.append(arr1)
        ...
        if (i % 20 == 0) or (i == num_steps - 1):load swap file neovim
