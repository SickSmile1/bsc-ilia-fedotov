...
def plot_mfl(pt, pt2, file, title,ax, yl='Velocity', legend=True):
    ...
    if not legend:
        fig.colorbar(neg, ax=ax, location='right', anchor=(0, 0.3), shrink=0.7, label="Mass flow rate [UH]")
        ax.set_xlabel(r'$\Delta P_{SD}$ [$\frac{\mu U}{H}$]')
    
    if legend:
        fig.colorbar(neg, ax=ax, location='right', anchor=(0, 0.3), shrink=0.7)
        ax.set_ylabel(r'Obstacle r [H]')#, fontsize=14)
...
ax[0], arr, radii, press  = plot_mfl(pat, pat2, "/massflowrate_0.10/massflowrate.txt", "Massflowrate",ax[0], yl='P delta', legend=True)
#fig.savefig("flux_1.pdf", format='pdf', dpi=300, bbox_inches='tight')
#fig, ax2  = ret_fig_ax()
ax[1], arr2, radii2, press2  = plot_mfl(pat, pat2, "/massflowrate_0.50/massflowrate.txt", "Massflowrate",ax[1], yl='P delta', legend=False)
ax[0].text(-.18, 1.0, '(a)', transform=ax[0].transAxes, fontsize=14, fontweight='bold', va='top')
ax[1].text(-.18, 1.0, '(b)', transform=ax[1].transAxes, fontsize=14, fontweight='bold', va='top')
...
def plot_pres(pt, pt2, file, title,ax, yl='Velocity', legend=True):
    ...
    if not legend:
        fig.colorbar(neg, ax=ax, location='right', anchor=(0, 0.3), shrink=0.7, label=r'Pressure [$\frac{\mu U}{H}$]')
        ax.set_xlabel(r'$\Delta P_{SD}$ [$\frac{\mu U}{H}$]')
...
fig, ax = ret_fig_ax(1,2)
ax3, arr,radii, press  = plot_pres(pat, pat2, "/p_at_5_0.10/p_at_5.txt", "Pressure",ax[0], yl='P delta', legend=True)
#fig.savefig("pressure_1.pdf", format='pdf', dpi=300, bbox_inches='tight')

ax2, arr2, radii2, press2  = plot_pres(pat, pat2, "/p_at_5_0.60/p_at_5.txt", "Pressure",ax[1], yl='P delta', legend=False)
ax[0].text(-.18, 1.0, '(a)', transform=ax[0].transAxes, fontsize=14, fontweight='bold', va='top')
ax[1].text(-.18, 1.0, '(b)', transform=ax[1].transAxes, fontsize=14, fontweight='bold', va='top')
...
plt.ylabel(r'Mass flow rate Q [UH]')#, fontsize=14)
plt.xlabel(r'$\Delta P_{SD}$ [$\frac{\mu U}{H}$]')#, fontsize=14)
plt.xlim(550,650)
plt.ylim(2,5.5)
...
def plot_y(pt, pt2, file, title,ax, yl='Velocity', legend=True):
    ...
    if not legend:
        fig.colorbar(neg, ax=ax, location='right', anchor=(0, 0.3), shrink=0.7, label=r'Velocity [U]')
        ax.set_xlabel(r'$\Delta P_{SD}$ [$\frac{\mu U}{H}$]')
...
fig, ax = ret_fig_ax(1,2)
ax3, arr,radii, press  = plot_y(pat, pat2, "/y_at_5_0.10/y_at_5.txt", "Pressure",ax[0], yl='P delta', legend=True)
#fig.savefig("velo_1.pdf", format='pdf', dpi=300, bbox_inches='tight')
#fig, ax2  = ret_fig_ax()
ax[0].text(-.18, 1.0, '(a)', transform=ax[0].transAxes, fontsize=14, fontweight='bold', va='top')
ax[1].text(-.18, 1.0, '(b)', transform=ax[1].transAxes, fontsize=14, fontweight='bold', va='top')
ax2, arr2, radii2, press2  = plot_y(pat, pat2, "/y_at_5_0.60/y_at_5.txt", "Pressure",ax[1], yl='P delta', legend=False)
...
ax.plot(arr2[0],label="simulated")
...
print("abs mean dist: ",np.mean( np.abs(arr2[0] - res) ) )
print("linalg norm: ",np.linalg.norm(arr2[0] - res))

def calculate_percentage_deviation(array1, array2):
    return 100 * (array2 - array1) / ((array2 + array1) / 2)

print(calculate_percentage_deviation(arr2[0], res))
