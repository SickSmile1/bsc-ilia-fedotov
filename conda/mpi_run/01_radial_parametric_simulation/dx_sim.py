...
def run_sim(comm, height=1, length=3,pres=10,T=.8,num_steps=1000,r=0, save=False, tol=.05, mesh_created=False, meshed=None):
    # set obstacle location to center
    Ox = length/2
    # enable/disable saving to .bp file, usually to debug
    file = False
    ...
    # Define boundary conditions for walls, inlet and outlet
    ...
    # this value can be used to break the run if the mass flow rate change falls below 3e-3 in the loop
    mfl_old, mfl = 0,0
    ...
    # create an array along the obstacle to evaluate pressure and velocity
    def half_circle_down(r, num_points=100):
        return [length/2+r * np.cos(np.linspace(0, np.pi, num_points)), (1-tol)-r * np.sin(np.linspace(0, np.pi, num_points)),
           np.zeros(num_points)]
    # arrays to calculate pressure at membrane, inlet, outlet
    ...
    for i in range(num_steps):
        ...
        if (i !=0 and i!=1 and (i%200)==0): # and comm.rank == 0:
            ...
            # sort the arrays, if they are mixed up by calculations on different cores
