...
def mfl_press(comm,x_max, mesh, mesh_tag, u_n, p):
    """
    Calculate mass flow rates and average pressures at specific slices of the mesh.

    This function computes the mass flow rate and average pressure at three slices
    along the x-axis of the mesh: at x=0.5, x=x_max/2, and x=x_max-0.5.
    ...
    mesh_tag (MeshFunction):
        Mesh function for subdomains (currently unused)
    u_n (Function):
    ...
    tuple: 
        ...
        - mfl (numpy.ndarray): Mass flow rates at the three slices
        - pressure_avg (numpy.ndarray): Average pressures at the three slices

    Note:
    This function uses MPI for parallel computations and prints the mass flow rates
    only on the root process (rank 0).
    """
...
def get_unsorted_arrays(pop, cell, u_n, p_n): #, pop, cell):
    # Find cells whose bounding-box collide with the points on proc as in
    # the example of dokken: https://jsdokken.com/dolfinx-tutorial/chapter1/membrane_code.html
    if len(pop) > 0:
...
def gather_and_sort(pop, u_val, p_val, mesh):
    ...
    if mesh.comm.rank == 0:
        # measuring accuray is 100 points for a slice: 
        #   recombine, if array is < 90 -> split into processes 
        if len(all_pop) < 90:
            ...
            # Create a sorting index based on pop[:,0]
...
def write_x_parview(msh,ct,ft, name):
    """
    Write mesh and meshtags to an XDMF file for ParaView visualization.

    This function writes the mesh, cell tags (ct), and facet tags (ft) to an XDMF file
    for each MPI rank. The output files are stored in the 'out_gmsh' directory.

    Parameters:
    msh (dolfinx.mesh.Mesh): The mesh to be written.
    ct (dolfinx.mesh.MeshTags): Cell tags associated with the mesh.
    ft (dolfinx.mesh.MeshTags): Facet tags associated with the mesh.
    name (str): Base name for the output file and XML paths.
...
def store_gmsh(model, name, path):
    """
    Store a Gmsh model to a specified path.

    This function saves a Gmsh model to a file with the given name in the specified path.
    If the model is None, it prints a debug message instead.

    Parameters:
    model (gmsh.model): The Gmsh model to be stored. Can be None.
    name (str): Name of the output file (without extension).
    path (str): Base path where the file will be stored.
    db (str, optional): Database name. Defaults to "dtool_db".
...
def store_array(arr, name, path, t):
    """
