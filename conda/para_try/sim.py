...
def mfl_press(x_max, mesh, u_n, p):
    ...
    print("<< starting loop inside mfl_press >>")
    for i in pos:
        ...
        #pa = MPI.COMM_WORLD.gather(pa, root=0)
...
def create_progress_bar(total, desc="Progress"):
    return tqdm(total=total, desc=desc, disable=MPI.COMM_WORLD.rank!=0)

def update_progress_bar(pbar, amount=1):
    if pbar is not None:
        pbar.update(amount)
...
def run_sim(height=1, length=3,pres=8,T=.5,num_steps=500,r=0, file=False, run=1, tol=.07):
    ...
    if debug:
        print("<< starting loop >>")
    pbar = tqdm.tqdm(total = num_steps, desc = "Timesteps")
    ...
    solver2.destroy()
    solver3.destroy()
    print(f'<< finished run with {pres:d} >>')
    return u_n, p_n, V, mesh

if __name__ == '__main__':
    pbar1 = tqdm.tqdm(total = len(np.linspace(0.05,0.5,19)), desc = "radii")
    for i in np.linspace(0.05,0.5,19):
        for pres in [2,4,6,10,12,14]: 
            if i==0.05 and pres<=4:
                continue
            print(f"\n\n<< starting to calculate pressure {pres:d} >>\n\n")
            height, length, T, num_steps, r, Ox, tol = 1,10,3,3000,i,5,.03

            if MPI.COMM_WORLD.rank == 0:
                p, pat = init_db(f"canal_{i:.2f}_{int(pres*length):d}", False)
                # annotate metadata : height=1, length=3,pres=8,T=.5,num_steps=500,r=0, file=False, run=1, tol=.07):
                p.put_annotation("metadata", write_values_to_json([height, length, pres*length, T, num_steps, r, Ox, tol],
                                                                  ["height", "length", "pressure_delta", "simulation_time", 
                                                                   "steps", "radius", "obstacle_location_x","meshing_size/tol"]) )
            u_, p_, V, mesh = run_sim(height,length,pres,T=3,num_steps=3000,r=i,file=False,run=2, tol=0.03)
            mfl = mfl_press(length, mesh, u_, p_) # comm.bcast(lower, root=0)
