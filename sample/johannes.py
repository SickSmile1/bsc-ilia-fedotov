...
# %%
mesh, ct, ft, infl, outfl, upper, lower = create_mesh(MPI.COMM_WORLD, H=1, L=1, lc=0.02)
...
left_boundary = ft.find(3)
right_boundary = ft.find(2)
upper_boundary = ft.find(1)
lower_boundary = ft.find(4)
...
for i in range(num_steps):
    ...
    if (i % 20 == 0) or (i == num_steps - 1):
        print(
            f"Time {t:.2f}, L2-error {error_L2:.2e}, Max error {error_max:.2e}, Flux left {mass_flow_left:.2e}, Flux right {mass_flow_right:.2e}, Flux upper {mass_flow_upper:.2e}, Flux lower {mass_flow_lower:.2e}, Flux center {mass_flow_through_cross_section:.2e}")        
# Close xmdf file
vtx_u.close()
vtx_p.close()
b1.destroy()
